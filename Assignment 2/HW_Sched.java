...
public class HW_Sched {
	...
	public int[] SelectAssignments() {
		...
		// Sort assignments with weights in descending order
		...
		for (int i = 0; i < Assignments.size(); ++i) {
			// assign an assignment to the latest available slot
			for (int j = Math.min(lastDeadline, Assignments.get(i).deadline) - 1; j >= 0; --j) {
				if (homeworkPlan[j] == -1) {
